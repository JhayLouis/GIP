...
export const validateCourseName = (courseName: string): { valid: boolean; error?: string } => {
  ...
  if (trimmed.length < 10) {
    return {
      valid: false,
      error: "Course name is too short (minimum 10 characters)"
    };
  }
  ...
  if (!hasValidPrefix) {
    return {
      valid: false,
      error: "Course must start with Bachelor of, Associate of, Diploma of, Certificate in, Master of, Doctor of, or Professional"
    };
  }
  ...
  if (hasNumbers) {
    return {
      valid: false,
      error: "Course name should not contain numbers"
    };
  }
